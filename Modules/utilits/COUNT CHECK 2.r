...
SQLite_path ="C:\\Users\\Admin\\Desktop\\138_2016-2020 NFS SSL.db"
...
##########################################################################
#photo_count_list=photo_count_list[photo_count_list$type=="Acount",]
...
if (type_count %in% c("opp_manual_full_count","opp_auto_count")) {
...
#####################################################				
if(type_count =="Acount") {	
	DayAccessCount=NULL
	DayAccessCount=data.frame(
	                        animal_type=photo_countSort$animal_type,
	                        local_site =photo_countSort$local_site,
	                        comments    ="",
	                        r_date  =photo_countSort$r_date,  
	                        time_start =photo_countSort$time_start,  
	                       observer   =photo_countSort$observer,
	                       species  =photo_countSort$species,  
	                       type_count=type_count)
	SeasonAccessCount =rbind(DayAccessCount,SeasonAccessCount)

	}	
}
################################################
countSum=rbind(SeasonModelCount,SeasonFullCount,SeasonAccessCount)	### SUM MODEL COUNT AND FULL COUNT 
