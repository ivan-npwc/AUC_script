...
library(magick)
 library(parallel)
  ...
  TblAgeRef=NULL
  Species="SSLAdult"
  saveFolder = "C:\\SSL_DB\\SSL_AUC\\20191218\\data\\TRAIN\\Age_SSL\\Original"
  DirOPP="D:\\2019_138_OPP"
...
for (g in 1:length(listOPP)) {
  ...
  PthObserverCount=paste0(labelInput,"\\Observer count\\", basename(labelInput), ".shp")
 ...
 #   AgeRefRow1=  foreach(i = 1:length(ImgList$link1)) %dopar% {
  ...
  AmimalsInside512=AmimalsInside512[AmimalsInside512$LAYER =="F",]
...
write.csv(TblAgeRef,"TblAgeRef.csv")