...
library(magick) 
	      ...
	      batch_size =64
	      mdlPTH = paste0(System_data,"/weights/NFS_AGE/NFSageBASE/NFS-SSL__20220902_loss_0.19_epoch_02.h5")
	      WeightPTH =  paste0(System_data,"/weights/NFS_AGE/NFS-SSL_vgg16" )
...
##################################################################################
	     if(exists("model")==F){model<<-load_model_hdf5(mdlPTH)}
...
###############################################################################################
	...
	names(preds3) <- c("NFS","SSL")
	preds3$link=lstimgs
for (i in 1:length(lstimgs)) {
   if(preds3$NFS[i]> preds3$SSL[i]){preds3$name[i]="NFS"}
   if(preds3$NFS[i]< preds3$SSL[i]){preds3$name[i]="SSL"}
   }
#################################################################################################
SSLimgs=preds3$link[preds3$name=="SSL"]
