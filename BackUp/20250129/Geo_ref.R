###########################################################################
	...
	if (file.exists(PthTable)==F){PthTable=paste0(labelInput,"\\", basename(date1), ".csv")}
					readPth=paste0(labelInput, "\\Predict\\", Species,"_BlobTable_",basename(date1), ".csv" )
	if (file.exists(readPth)==F){readPth=paste0(labelInput, "\\Predict\\", "NFSPup256_BlobTable_",basename(date1), ".csv" )}				
...
#  BlobTable=BlobTable[BlobTable$m.cx != 0,]
  ...
  dimModel=256     # 
  if (Species== "NFSPup512"){dimModel=512}
