...
##############################################################################  
  ...
  if(exists("unet1")==F) {
	...
	#if (Species == "NFSPup") {source("Modules/Unet512Create.r")}
	}
...
dice_coef <- 
 ...
 Preds1=list(preds=preds,DimPreds=dim(preds),dimModel="256",listImageBl=basename(listImageBl))
    ############
#	if (Species=="NFSPup"){
#	
#	source("E:\\GIT HUB\\AUC\\Modules\\PUP_NFS\\PredsMergNFSpup.r")
#	pthSavePreds=paste0(predsDir,"\\Preds_",Species,"_",e)
#	saveRDS(PredsNew1,pthSavePreds)
#	
#	} else {
