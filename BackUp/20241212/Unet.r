              ...
              library(unet)      
					labelInput 
					 ...
					 if (dir.exists(predict_dir)==F){predict_dir=paste0(labelInput,"\\Predict\\TilesOverlap")}
					 if (Species=="NFSPup512"){predict_dir=paste0(labelInput,"\\Predict\\NFSPup512")}
...
##############################################################################  
 ...
 # if(exists("unet1")==F & Species != "NFSPup512") {source("Modules/unetVGG16Create.r")} 
 # if(exists("unet1")==F & Species == "NFSPup512") {source("Modules/Unet512Create.r")}
 # if(exists("unet1")==F & Species == "NFSPup256") {unet1=unet(c(256,256,3))}
  unet1=unet(c(256,256,3))
...
# if (is.null(listImgdonePred)==F){listImage_glob<<-listImage_glob[!listImage_glob %in% listImgdonePred] }
 ...
 ###################################################### 
  ...
  #################################################################################
  for (e in 1:global_steps) { #global_steps
