UNET=function(Species){
					 ...
					 if (BLACKMATTER==T){predict_dir=paste0(labelInput,"\\Predict\\Image_enhancement")}
...
##############################################################################  
  ...
  if (Species == "NFSAdult") {weight = paste0(System_data,"/weights/",NFS_Adult_weight_pth)}
  if (Species == "SSLAdult") {weight = paste0(System_data,"/weights/",SSL_Adult_weight_pth)}
  if (Species == "SSLPup") {weight = paste0(System_data,"/weights/",SSL_Pup_weight_pth)}
  if (Species == "NFSPup256") {weight = paste0(System_data,"/weights/",NFS_Pup_weight_pth)}
  if (Species == "NFS_AN_TF_UNET") {weight = paste0(System_data,"/weights/",NFS_AN_TF_UNET)}


   set_weights(unet1,readRDS(weight))
...
UNET(Species= "NFSAdult")
print("NFSAdult is done")
UNET(Species = "SSLAdult")
print("SSLAdult is done")
UNET(Species = "SSLPup")
print("SSLPup is done")
UNET(Species = "NFSPup256")
print("NFSPup is done")

#UNET(Species = "NFS_AN_TF_UNET")
#print("NFS_AN_TF_UNET is done")
