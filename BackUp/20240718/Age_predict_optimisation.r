...
#source("Modules/KMLwrite_function.r")
 ...
 OPTPoligonDIR=paste0(labelInput,"\\Polygons\\Prediction optimisation")
 ModelPoligonDIR=paste0(labelInput,"\\Polygons\\Model")
  ...
  OPTPoligonPTH=list.files(OPTPoligonDIR,full.names=T,pattern="shp|kml")
   ...
   proj4string(polygon_exlude) <- crs	
    ...
    data   <- data.frame(AN=APO$AN, Bch=APO$Bch, Rookery=APO$Rookery,name = APO$name)   # data
      ...
      APO1 <- SpatialPointsDataFrame(coords = coords,
		...
		ModelPoligon0=readOGR(ModelPoligonPTH)
		OPTPoligon1=readOGR(OPTPoligonPTH)
		
		ModelPoligon1=rbind(ModelPoligon0,OPTPoligon1)
