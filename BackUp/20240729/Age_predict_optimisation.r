...
#source("Modules/KMLwrite_function.r")
 ...
 # save_pth_check_diff=paste0(labelInput,"\\Predict\\Check_difference",Species,"_", date1, ".csv")
  RookeryPoligonDIR=paste0(labelInput,"\\Polygons\\Rookery")
  ObserverPointDIR=paste0(labelInput,"\\Observer_count")
  Exlude_polygonDir=paste0(labelInput, "\\Polygons\\Exclude")
  
   if(dir.exists(Exlude_polygonDir)==T){Exlude_polygon_pth<-list.files(Exlude_polygonDir,full.names=T,pattern="shp|kml");polygon_exlude=readOGR(Exlude_polygon_pth)}
  ObserverPointPTH=list.files(ObserverPointDIR,full.names=T,pattern="shp|kml")[1] 
  RookeryPoligonPTH=list.files(RookeryPoligonDIR,full.names=T,pattern="shp|kml")
  RookeryPoligon=readOGR(RookeryPoligonPTH)
   ...
   proj4string(polygon_exlude) <- crs	
    ...
    data   <- data.frame(AN=APO$AN, Bch=APO$Bch, Rookery=APO$Rookery,name = APO$name)   # data
      ...
      APO1 <- SpatialPointsDataFrame(coords = coords,
		...
		proj4string(RookeryPoligon) <- crs	
        RookeryPoligon=	spTransform(RookeryPoligon,crs)		
       
	    proj4string(Observer) <- crs
        Observer=	spTransform(Observer,crs)
		
   
  APO3= APO2[!is.na(over(APO2,as(RookeryPoligon,"SpatialPolygons"))),]
  OBS = Observer[!is.na(over(Observer,as(RookeryPoligon,"SpatialPolygons"))),]
