...
Species
  ...
  hdr=paste0(ageFolder,"\\Rookery");unlink(hdr);dir.create(hdr)
  rdr=paste0(ageFolder,"\\Haulout");unlink(rdr);dir.create(rdr)
   ...
   imgDraw=image_draw(img2)
	...
	abline(v=x,col=2,lwd=10)
	 dev.off()
	  ...
	  if (Species=="SSLPup") {size_crop=paste0("256x256+",x-128, "+",y-128) }
