   ...
   labelInput="E:\\NFS_DB\\2022_138_OPP\\20220711_074952"
   Species="NFSadult"
   date1<<-substr(basename(labelInput),1,15)
   HaulAsModel
   ObserverErrCompensate=T
   kmlCheck=F
...
##############################################################################################################		
 if (Species %in% c("NFSadult" ,"SSLAdult","NFSPup")) {		
		  ...
		  RookeryPol=readOGR(RookeryPolygonPTH)
...
if (kmlCheck==T){
		  ...
		  writeOGR(ObserverPoint,  "observer.kml", "age", "KML")
	      writeOGR(ModelPoligon1,  "pol.kml", "pol", "KML")
