Image_prepare=function (labelInput1=labelInput) {
  ...
  library(spatialEco)
  Tpth=paste0(labelInput1,"\\", basename(labelInput1), "_table.csv")
...
if (file.exists(Haulout_polygon1) == F) {stop("Please create polygon animal presence and place it  in your dir ~/Polygons/Haulout/Haulout.shp")}
   ...
   CombinationOne=data.frame(lat= table_img$west, lon=table_img$south,link= table_img$link)
   CombinationTo=data.frame(lat= table_img$west, lon=table_img$north,link= table_img$link)
   CombinationThry=data.frame(lat= table_img$east, lon=table_img$south,link= table_img$link)
   CombinationFor=data.frame(lat= table_img$east, lon=table_img$south,link= table_img$link)
   PointsPorderImage=rbind(CombinationOne,CombinationTo,CombinationThry,CombinationFor)
    ...
    PointsImg <- SpatialPointsDataFrame(coords = coords,
                                     data = data, 
                                     proj4string = crs)
  # if (file.exists(Haulout_polygon1)==F) {
   
         Haulout_polygon_poly=shapefile(Haulout_polygon1) # need kml
		
		Index=  PointsImg %over%  Haulout_polygon_poly 
		PointsImg$Index= Index[,1]
		resLink=  data.frame(link=unique(PointsImg$link[is.na(PointsImg$Index)==F]))
...
#               resLink=rbind(HauloutImg3,resLink)
#  } 
  }
 ############################################################################################################################# 
  KMLpath<<-paste0(labelInput1,"\\",basename(labelInput1))
  ImgSave=paste0(labelInput1, "\\", "Predict", "\\","Haulout") 
  subDir=paste0(labelInput1, "\\", "Predict")
  ...
  if (Need !=Presence) {
    ...
    foreach(i = 1:length(resLink$link)) %dopar% {	
      #	for (i in 1:length(table$link)) {
      selectRow<-table[i,]	
	  if (KMLfocusPocus==T) {selectRow <- table[table$link== resLink$link[i],]}
	...
	#  pthExpImg=gsub("png","jpg",pthExpImg)  # i need first retrain the model on jpg images or not.....let's tray
