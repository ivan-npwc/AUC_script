...
#PredsPrepareSQL=function (labelinput1) { #labelinput1 - it is subfolder in general labelInput folder
    ...
    source("Modules/GetAgisoftShape.r")	
     ...
     Haulout_polygonDir=paste0(labelinput1, "\\Polygons\\Haulout");HPF<-list.files(Haulout_polygonDir,full.names=T,pattern="shp|kml")
    if (length(HPF)==0){stop("No Haulout Polygon found")}
 	 Haulout_polygon=readOGR(HPF)  
   ...
   if (Count_type %in% c("manual_count_model","manual_count_full")) { ################################### data for manual count
	...
	Count=NULL
	  for(i in 1:length(ObserverCountP)) {
		...
		ObsCount=NULL
		if (length(pts1$LAYER.x) != 0){ObsCount=data.frame(lon=pts1$coords.x1, lat= pts1$coords.x2,age= pts1$LAYER.x,local_site=pts1$LAYER.y, creator=observer)}
		if (length(pts1$Name.y) != 0){ObsCount=data.frame(lon=pts1$coords.x1, lat= pts1$coords.x2,age= pts1$Description.x,local_site=pts1$Name.y, creator=observer)}
		if (length(pts1$Name) != 0){ObsCount=data.frame(lon=pts1$coords.x1, lat= pts1$coords.x2,age= pts1$LAYER,local_site=pts1$Name, creator=observer)}
