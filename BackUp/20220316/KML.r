...
#library(spatialEco)	
				...
				#	if (Species=="WLRS"){clastering=T}		 
					...
					kmlPathSave1 <- paste0(labelInput,"\\Predict\\",Species, "_", date1, ".kml")
...
RP=Pointsfilter(tble=dat3,pthPolygon=Rookery_polygon)
  RP1=data.frame(RP)
...
######################################################################################################### 	
PointsToWrite=rbind(PointsHoulout2,RookeryPoints,PupPoints3)
