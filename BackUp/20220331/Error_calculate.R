...
ObserverPointPTH=list.files(ObserverPointDIR,full.names=T,pattern="shp|kml")[1] 
  ...
  if (length(ModelPoligonPTH) == 1  & is.na(ObserverPointPTH) == F & file.exists(PredictPointPTH_kml)) {
##################################################################################################################				
##############################################################################################################		
 if (Species %in% c("NFSAdult" ,"SSLAdult")) {		
		  ...
		  ObserverCount$age= gsub("SSL_AN","AN",ObserverCount$age)
		  ObserverCount$age= gsub("SSL_TN","TF",ObserverCount$age)
		  ObserverCount$age= gsub("SSL_TF","TF",ObserverCount$age)
		  ObserverCount$age= gsub("SSL_F","F",ObserverCount$age)
		  ObserverCount$age= gsub("SSL_D","U",ObserverCount$age)
		  ObserverCount$age= gsub("SSL_UNK","U",ObserverCount$age)
		  ObserverCount$age= gsub("SSL_SA","SA",ObserverCount$age)
		  ObserverCount$age= gsub("SSL_J","J",ObserverCount$age)
		  ObserverCount$age= gsub("SSL_P","P",ObserverCount$age)
		  
		  ObserverCount$age= gsub("TN","TF",ObserverCount$age)
...
####################################################################################################### NO OBSERVER COUNT NO AUTO COUNT RESULTS= NO ANIMALS
 ...
 ########################################################################################### NO OBSERVER COUNT, BUT AUTO COUNT EXISTS
  if (is.na(ObserverPointPTH) == T & file.exists(PredictPointPTH_kml)) {
          ...
          Compare1=data.frame(age=PredictCount1$age, AutoCount=PredictCount1$count, ObserverCount=0, diffInd=PredictCount1$count, diffPerc=NA,date1=date1, Species=Species)
  
  }
 ################################################################################## 
  if (is.na(ObserverPointPTH) == F & file.exists(PredictPointPTH_kml)) {   # no auto count but observer count exists
