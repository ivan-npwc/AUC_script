...
#PredsPrepareSQL=function (labelinput1) { #labelinput1 - it is subfolder in general labelInput folder
  ...
  options(warn=-1)
   if (Count_type %in% c("manual_count_model","manual_count_full")) { ################################### data for manual count
    ...
    ############################################################################### ADD MODEL POLYGONS
    ###############################################################################
    if (Count_type ==	"manual_count_model" & file.exists(MdlPolPth,showWarnings=F)  & file.exists(ObserverCountP,showWarnings=F)) { 
...
if (is.null(Count) == F) { # only if  data in Count
 ...
 Row = Effort[Effort$observer==observer & Effort$date== datetime & Effort$type_count== Count_type,]
