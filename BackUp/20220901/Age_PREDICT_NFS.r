...
Age_Pred_Fun=function(
	...
	#check=F,
	month1,
	R_mdlBSAgePTH = paste0(System_data,"/weights/NFS_AGE/NFSageBASE/TF_F__20220819_loss_0.13_epoch_01.h5"),
	H_mdlBSAgePTH =paste0(System_data,"/weights/NFS_AGE/NFSageBASE/TF_F__20220819_loss_0.13_epoch_01.h5"),
...
###############################################################################################
if (type=="Rookery"){	preds3=data.frame(pred)
	names(preds3) <- c("F", "TF")
...
for (i in 1:length(listImgPred)) {
   ...
   #################################################################################
   if (type=="Haulout"){	preds3=data.frame(pred)
	names(preds3) <- c("AN", "Bch")
	preds3$link=basename(as.character(listImgPred))
for (i in 1:length(listImgPred)) {
   if(preds3$AN[i]> preds3$Bch[i]){preds3$name[i]="AN"}
   if(preds3$AN[i]< preds3$Bch[i]){preds3$name[i]="Bch"}
   }
   }
#################################################################################################
