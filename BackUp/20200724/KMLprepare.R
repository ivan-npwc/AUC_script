...
######################################################################
KMLprepare=function () {
  to1=  paste0(labelInput,"\\",basename(labelInput))
  ...
  if (check3=="d") {
   ...
   if (!check3 %in% c("e","c","d","b")) {source("Modules/02_generate_image_anchors.r")}
  ...
  KMLimg$format.img= extension(KMLimg$link) 
 ...
 foreach(e = 1:length(KMLimg$link)) %dopar% {	
  ...
  table1$date=basename(labelInput)
