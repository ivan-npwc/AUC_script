...
##########################################################   
  if (Species=="SSLAdult") {
            ...
            #            oldImgDir=paste0(labelInput,"\\Predict\\Age_predict")
			#			 newImgDir=paste0(labelInput,"\\Predict\\SSLAdult")
	
			#			 dir.create(newImgDir)
			#			 foldercopu=list.files(oldImgDir, full.names=T)
			#			 file.copy(foldercopu,newImgDir, recursive=T)
...
PointsToWrite=PointsToWrite[!PointsToWrite$age %in% c("SmallError","BigError"),]
