...
# Parameters -----------------------------------------------------
	...
	Weight = "D:\\MachineLearning\\AUC_script\\System data\\weights\\WLRS"
	epochs =40
	batch_size=16L	
...
unet1 <- unet1 %>%
    compile(
        optimizer = optimizer_adam(lr= 2e-6),# optimizer_adam(lr= 0.0001, decay = 1e-6 )  optimizer_rmsprop(lr = 2e-5)
        loss =   dice_coef_loss,    
        metrics = c(dice_coef) 
   )
########################################	
