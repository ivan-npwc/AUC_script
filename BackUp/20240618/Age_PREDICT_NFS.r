...
for (i in 1:length(listImgPred)) {
 ...
 ######################################
 if (type=="Haulout"){preds3$name[preds3$AN < 0.95]="Bch"} # try 85
...
coords <- data.frame(lat= Hl$lon, lon=Hl$lat)   
 ...
 rk=data.frame(Pointsfilter(tble=Hl,pthPolygon= Rookery_polygon, IN="in")) 
 
  if (!is.null(rk)) {rk$age[rk$age=="Bch"]="F";rk$age[rk$age=="AN"]="TF"}
 
 
 Houl=data.frame(Pointsfilter(tble=Hl,pthPolygon=Rookery_polygon, IN="out"))
