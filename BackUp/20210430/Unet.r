...
#unet1_Pred_Fun=function(
                     labelInput1="D:\\SSL_DB\\2020_138_OPP\\20200614_084128"  #labelInput
                      Species1=Species
				...
				#	  ) {
                        ...
                        date1=basename(paste0(labelInput1))
						predict_dir=paste0(labelInput1,"\\Predict\\Haulout")

##############################################################################################################################
if (Species1== "NFSAdult") {PTHweight=NFS_Adult_weight_pth }		
if (Species1== "SSLAdult") {PTHweight=SSL_Adult_weight_pth}	
if (Species1== "NFSPup") { PTHweight=NFS_Pup_weight_pth }
if (Species1== "SSLPup") { PTHweight=SSL_Pup_weight_pth}
if (Species1== "WLRS")   {PTHweight=WLRS_weight_pth}

    PTHweight<-paste0("System data/weights/",PTHweight)
    predsDir=paste0(labelInput1,"\\Predict\\Preds")
	
	if(dir.exists(predsDir)==T){ listPredsDelete=list.files(predsDir,full.names=T,pattern= Species1);unlink(listPredsDelete) }else{dir.create(predsDir)}
  ...
  if(exists("unet1")==F) {
    if (Species1 != "NFSPup") {source("Modules/unetVGG16Create.r")}
	if (Species1 == "NFSPup") {source("Modules/Unet512Create.r")}
	}
  ...
  pth_resultBlob <<- paste0(labelInput1,"\\Predict\\",Species1,"_BlobTable_", date1, ".csv") 
  pth_resultBlob_tmp <<-paste0(labelInput1,"\\Predict\\",Species1, "_BlobTable_", date1, "_tmp.csv")
...
#	batch=iter_next(test_iterator)
 if (tensorflow::tf_version() <= "2.0") {
    ...
    ############
	pthSavePreds=paste0(predsDir,"\\Preds_",Species1,"_",e)
