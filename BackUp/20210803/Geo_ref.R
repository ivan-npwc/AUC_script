###########################################################################
					...
					clastering= T # if need clastering points based on distance 
...
#  BlobTable=BlobTable[BlobTable$m.cx != 0,]
 ...
 # tableBlob1<<-tableBlob1
  BlobTable<<-BlobTable
  BloblistImg<<-unique(BlobTable$img)
  for (j in 1:length(BloblistImg)) { 
    imgN<-paste0(BloblistImg[j])
	ImgFormat= extension(imgN)
    BlobImgTable<-BlobTable[BlobTable$img==imgN,]
	if (extension(imgN) != extension(tableBlob1$link[1])) {extension(tableBlob1$link)=extension(imgN)}
    refTable<-tableBlob1[tableBlob1$link==imgN,]
...
##################################################################
  ...
  result_geo3$DimModel=dimModel
 # result_geo3=result_geo2[result_geo2$m.cx >= IndexBorderLeft & result_geo2$m.cx <= IndexBorderRight,] # filter only central part
 # result_geo4=result_geo2[result_geo3$m.cy >= IndexBorderLeft & result_geo3$m.cy <= IndexBorderRight,] 
  write.csv(result_geo3, savePth,row.names = F)
   predsDir=paste0(labelInput,"\\Predict\\Preds")
   if (deletePreds==T) {unlink(predsDir,recursive=T)}
