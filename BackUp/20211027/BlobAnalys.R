  ...
  for (f in 1:length(listPreds)) {
   ...
   cl <- makePSOCKcluster(detectCores ()) 
    ...
    registerDoParallel(cl)
       ...
       pth_resultBlob <<- paste0(labelInput1,"\\Predict\\",Species,"_BlobTable_", date1, ".csv") 
       pth_resultBlob_tmp <<-paste0(labelInput1,"\\Predict\\",Species, "_BlobTable_", date1, "_tmp.csv")
