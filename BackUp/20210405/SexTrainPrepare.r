...
library(magick)
 library(parallel)
  ...
  TblAgeRef=NULL
  Species="SSLAdult"
  saveFolder = "E:\\AUC_data\\SSL_age\\NewData"
  DirOPP="E:\\SSL_DB\\2020_138_OPP"
 SelectedCategory="J"
 TblAgeRefSave=   paste0(saveFolder,"\\TblAgeRef_",SelectedCategory, ".csv")  
...
for (g in 1:length(listOPP)) {
  ...
  PthObserverCount=paste0(labelInput,"\\Observer count\\", date1, ".shp")
 ...
 #   AgeRefRow1=  foreach(i = 1:length(ImgList$link1)) %dopar% {
  ...
  AmimalsInside512=AmimalsInside512[AmimalsInside512$LAYER ==SelectedCategory,]
...
if (length(AmimalsInside512$coords.x1)>0 ) {
	...
	pth_save_img=paste0(save_folder_sex,"\\",sex,f,"_",imgName)
...
AgeSmallFolder = paste0(saveFolder,"\\Small"); if (dir.exists(AgeSmallFolder)==F) {dir.create(AgeSmallFolder)}
  listImgsPTH=unique(tableRef1$pth_img)
   ...
   if (file.exists(pth)) {
       age=SelectedCategory #paste0(tableRef1$age[i])
       x=row1$x-256
       y=row1$y-256
