...
##################################################################################################################				
##############################################################################################################		
 if (Species %in% c("NFSAdult" ,"SSLAdult","NFSPup")) {		
		 ...
		 if(Species=="NFSPup"){PredictPoint1=PredictPoint1[PredictPoint1$Description=="P",]} 
		  ...
		  ObserverCount = ObserverPoint[!is.na(over(ObserverPoint,as(ModelPoligon1,"SpatialPolygons"))),]
           ...
           if (length(ObserverCount)==0) {ObserverCount=data.frame(age=NA)}			  
		  ...
		  ObserverCount$age= gsub("OM","AN",ObserverCount$age)
		  ObserverCount$age= gsub("Sa","SA",ObserverCount$age)
		  ObserverCount$age= gsub("An","AN",ObserverCount$age)
	if (Species==  "NFSAdult") {
