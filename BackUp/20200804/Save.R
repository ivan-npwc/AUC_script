...
PredsWriteToSQL=function (labelinput1,
 ...
 Species1<<-Species
   ...
   if (Species1=="SSLPup")   {stop("You can not Import SSL pups witout adult, please select  SSLAdult")}
...
#####################################################################################
 ...
 if (EffortSort$type_count=="Auto_count") {  # data for auto count
  ...
  if (EffortSort$type_count %in% c("Manual_full_count","Manual_model_count")) { # data for manual count
               if (file.exists(ObserverCountP)==F) {stop(paste0("No observer count data, but Effort exists ",labelinput1))}
