...
#					   ) {
                      ...
                      trainDir1=trainDir
					   ...
					   dfrmn=F
                      TypeTrain
                      DataLimit="0720"  # use img only untill 15 july
...
######################################
listImgS <- list.files(images_dir, full.names=T)
listImgs1=data.frame(FullPth=listImgs, Nmae=basename(listImgs))

listImgs1$date=substr(listImgs1$Nmae ,8,15)
listImgs1$MonthDay=substr(listImgs1$date ,5,8)
listImgs2= listImgs1$FullPth[listImgs1$MonthDay < DataLimit]
