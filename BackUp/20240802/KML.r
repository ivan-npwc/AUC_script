...
#library(spatialEco)	
 ...
 if (Species =="NFSAdult") {BlobFemaleLimit=500;BlobSAlimit=500; limitSmallError=10}
 
 if (Species =="SSLAdult") {BlobFemaleLimit=1200;BlobSAlimit=1200; limitSmallError=100}
 if (Species =="NFSPup256") {BlobFemaleLimit=70}
 if (Species =="NFSPup512") {BlobFemaleLimit=70}
...
coords <- data.frame(lat= tble$lat, lon=tble$lon)   
 ...
 ###################################################################################################################### EXLUDE POLIGON for all
    ...
    proj4string(polygon_exlude_polygon) <- crs		
...
################################################################################################################# ROOKERY SSL NFS PUP   
   PupPoints1=NULL
   if (Species %in% c("NFSPup512","NFSPup256")) {
...
#PupPoints3=rbind(PupPoints2,adults1)
    writeOGR(PupPoints,kmlPathSave,driver="KML", layer="Description")
#KMLwrite(Img3=PupPoints2,kmlPathSave=kmlPathSave)
	}
######################################################################################################### 	
PointsToWrite <<- rbind(PointsHoulout2,RookeryPoints,PupPoints3,PupPoints1)
write.csv(PointsToWrite,PthPredictCount, row.names=F)

   coords <- data.frame(lat= PointsToWrite$lon,lon=PointsToWrite$lat)                                                                 #   
    data   <- data.frame(Description= PointsToWrite$age)   # data                                                                      #
    PointsW <- SpatialPointsDataFrame(coords = coords,
                                     data = data, 
                                     proj4string = crs)
									 
	PointsW =spTransform(PointsW,crs)
	unlink(kmlPathSave)
    writeOGR(PointsW,kmlPathSave,driver="KML", layer="Description")
print("KML done")
#if (!Species %in% c("NFSPup512","NFSPup256")){source("C:\\Users\\usato\\SSL_DB\\AUC\\Modules\\Error_calculate.R")}
