...
# Parameters -----------------------------------------------------
	...
	trainDir = "D:\\SSL_DB\\TRAIN\\WLRS\\RockyTerrain"
	Weight = "D:\\SSL_DB\\TRAIN\\WLRS\\RockyTerrain\\Checkpoints\\RockyTerrain_20210803_Val_0.49_epoch_40_256"
	epochs =100
	batch_size=16L	
...
unet1 <- unet1 %>%
    compile(
        optimizer = optimizer_adam(lr= 2e-7),# optimizer_adam(lr= 0.0001, decay = 1e-6 )  optimizer_rmsprop(lr = 2e-5)
        loss =   dice_coef_loss,    
        metrics = c(dice_coef) 
   )
########################################	
