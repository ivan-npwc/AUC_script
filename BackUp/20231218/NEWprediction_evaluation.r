...
library(magick)
	...
	#    if(!file.exists(paste0(labelInput,"\\report_",date_1,".csv"))) {
		...
		ObserverCountPTH=paste0(labelInput, "/Observer count/",date_1,".shp")
    	if (dir.exists(predsDir)==T & file.exists(ObserverCountPTH)==T) {
	    labelInput=normalizePath(labelInput,winslash="/")
		
	    ModelPolPTH=paste0(labelInput, "/Polygons/Model/Model.shp")
        HaulPolPTH=paste0(labelInput, "/Polygons/Houlout/Houlout.shp") 
		ExludePolPTH=paste0(labelInput, "/Polygons/Exlude/Exlude.shp") #note Exclude not Exlude
		
		if (Species1=="SSLAdult") {ModelPolPTH<<-HaulPolPTH}
...
#	if(!check_rda_integrity(paste0(labelInput,"/Predict/",date_1,".rda")))
  #  if (ReAvaluation==T){
	...
	#		
	#	}else{
			kml_points=rgdal::readOGR(ObserverCountPTH)
	...
	retrieve_mask=function(ObserverCountPTH)
	{
	model_mask=NULL
		if(file.exists(ObserverCountPTH))
		{

			#if(!"Point Features"%in%rgdal::ogrListLayers(ObserverCountPTH)) stop(paste("There is no points in kml file:",kml_file))
			shape_1=rgdal::readOGR(ObserverCountPTH)
	...
	read_shape=function(ObserverCountPTH)
	{
	shape_1=NULL
		if(file.exists(ObserverCountPTH))
		{
		...
		model_mask=read_shape(ModelPolPTH)
		
		model_area=model_mask@polygons[[1]]@area
		
		haulout_mask=read_shape(HaulPolPTH)
	...
	#	shape_file=paste0(labelInput, "/Polygons/Model/Model.shp")
		model_mask=retrieve_mask(ModelPolPTH)
		}else{
			model_mask=NULL
		...
		if(!is.null(model_mask))
		{
			...
			exclude_mask=read_shape(shape_file=ExludePolPTH)
