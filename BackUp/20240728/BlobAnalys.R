	...
	# if (Species=="NFSPup3") {source("Modules/BlobAnalysNFSPup.r")} else {
	 ...
	 Brush=7
	 thresh=0.009
	 if (Species =="NFSPup256") {Brush=5;thresh=0.009}
     if (Species =="NFSPup512") {Brush=1}
 ...
 #  for (i in 1: length (listImageBl)) {
     ...
     img = getFrame(img0, 1)
       ...
       nmask2 = opening(nmask1, makeBrush(Brush,shape='disc') ) # shape='Gaussian', sigma=50
			  ...
			  if (Species == "NFSPup256") {nmask2 = dilate(nmask2, makeBrush(3	,shape='disc'))}
