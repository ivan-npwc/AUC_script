...
limitAutoMask=25 #no auto mask for more then 25 Animals
  ...
  ImgDir=paste0(labelInput,"\\Error\\Image")
  MskDir=paste0(labelInput,"\\Error\\Mask")
 
  } 
  if (UseAllHauloutImages==T){ 
  HauloutDir=paste0(labelInput,"\\Predict\\Haulout")
  listImgsHaul=list.files(HauloutDir,full.names=T)
  dir.create(paste0(labelInput,"\\Error"), showWarnings = F)
  ImgDir=paste0(labelInput,"\\Error\\Image"); dir.create(ImgDir, showWarnings = F)
  MskDir=paste0(labelInput,"\\Error\\Mask"); dir.create(MskDir, showWarnings = F)
...
DirImgs=paste0(labelInput,"\\Error\\Image")
