...
source("Modules/KMLwrite_function.r")
 ...
 #  difference=read.csv(save_pth_check_diff)
   ...
   Observer=readOGR(ObserverPointPTH)
    ...
    data   <- data.frame(AN=APO$AN, Bch=APO$Bch, Rookery=APO$Rookery)   # data

      APO2 <- SpatialPointsDataFrame(coords = coords,
                                        data = data, 
                                        proj4string = crs)
		APO2=	spTransform(APO2,crs)
        proj4string(APO2) <- crs
...
TFdiff<<-TFdiff
   ...
   LIMIT
	 ...
	 APO2$Bch=NULL  
	 APO2$Rookery=NULL 
     APO4=data.frame(APO2)
	 names(APO4)[2]= "lon"      
     names(APO4)[3]= "lat"  
   KMLwrite(Img3=APO4,kmlPathSave=kmlPathSave)
   
##   source("C:\\Users\\usato\\SSL_DB\\AUC\\Modules\\Error_calculate.R")
 #  error_calculate(FP_FN=FALSE)
