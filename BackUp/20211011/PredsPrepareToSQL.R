...
PredsPrepareSQL=function (labelinput1) { #labelinput1 - it is subfolder in general labelInput folder
  ...
  ##################################################################################### COLLECT data for auto count
   ...
   ################################################## ################################  COLLECT DATA MABUAL COUNT
   ##################################################################################
    if (Count_type %in% c("opp_manual_full_count","opp_manual_model_count","manual_count_full")) { ################################### data for manual count
    if (length(ObserverCountP)!=0 & file.exists(ObserverCountP,showWarnings=F)) {
        Points= readOGR(ObserverCountP)
		observer=strsplit(file_path_sans_ext(basename(ObserverCountP)),"_")[[1]][3]
