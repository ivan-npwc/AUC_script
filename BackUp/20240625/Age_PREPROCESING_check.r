...
System_data
  ...
  registerDoParallel(cl)
	...
	dirObsCount=paste0(labelInput,"\\Observer_count");pthObsCount=list.files(dirObsCount, full.names=T, pattern=".shp")

    HouloutFolder=paste0(labelInput,"\\Predict\\Haulout");listImgs=list.files(HouloutFolder, full.names=T)
    ageFolder=paste0(labelInput,"\\Predict\\", Species,"_check");dir.create(ageFolder)
	#unlink(ageFolder, recursive=T)
...
if (length(pthObsCount)>0){	
	...
	bsCnt=data.frame(readOGR(pthObsCount))
	
	####################
	Points <- SpatialPointsDataFrame(data.frame(bsCnt$coords.x1,bsCnt$coords.x2), data.frame(TrueAge=bsCnt$LAYER))
...
#########################################################################   
 ...
 # for (i in 1: length(ImgList$link1)) {
  TblAgeRef =	foreach(i = 1:length(ImgList$link1),.combine=rbind) %dopar% {
		...
		TrueAge=Points$TrueAge[is.na(Points$index)==F]
  ...
  if (length(AmimalsInside512[,1])>0 ) {
		 ...
		 AgeRefRow= data.frame(lon,lat,x=x,y=y,pth_img = pth_img,TrueAge=TrueAge)
