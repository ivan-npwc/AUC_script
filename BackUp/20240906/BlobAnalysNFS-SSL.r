	 ...
	 R_H_tbl_pth = paste0(labelInput,"\\Predict\\Rookery-Haul_split.csv")
	 
	 if (file.exists(R_H_tbl_pth)==F){source("C:\\Users\\usato\\SSL_DB\\AUC\\Modules\\ROOKERY-HOULOUT SPLIT IMGs.r")}
     R_H_tbl=read.csv(R_H_tbl_pth)
	 ...
	 listPredsNFSPup=list.files(predsDir,full.names=T,pattern="NFSPup256")
	# listPredsNFS_AN_TF_UNET=list.files(predsDir,full.names=T,pattern="NFS_AN_TF_UNET")
	 rk_imgs= unique(basename(R_H_tbl$pth[R_H_tbl$Rookery=="Rookery"]))
...
resultBlob_tmp <- foreach(i = 1:length(listImageBl_NFS),.combine=rbind) %dopar% {
 #  for (i in 1: length (listImageBl)) {
	...
	# imgPUP[imgPUP[,]  != 0] = 1
	 ...
	 imgSSL= dilate(imgSSL, makeBrush(37,shape='disc')) #  to reduce FP Haulout
	 imgPUP= dilate(imgPUP, makeBrush(43,shape='disc'))   #to reduce FP Bch
	# if FN female #imgNFSPup=imgNFSPup
	
	imgNFSPup = thresh(imgNFSPup, 18, 18, 0.009) 
    #imgNFSPup <- fillHull(imgNFSPup)
	#imgNFSPup = opening(imgNFSPup, makeBrush(5,shape='disc'))
	imgNFSPup= dilate(imgNFSPup, makeBrush(7,shape='disc'))   #  to reduce FP Female # if FP female #
	# 
	# 
    ...
    #   nmask2PUP = opening(nmask1PUP, makeBrush(7,shape='disc')) # shape='Gaussian', sigma=50
     #  nmask3PUP = fillHull(nmask2PUP)
	   ...
	   if (name %in% rk_imgs) { nmask3NFS = imgNFS  -  nmask3NFSPup}
	   if (!name %in% rk_imgs) {nmask3NFS = imgNFS  -  nmask3SSL-  imgPUP - nmask3NFSPup }
