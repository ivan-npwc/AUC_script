	...
	#  labelInput = "D:\\PV_DB/2023_H0099_OPP/20230502_135032/20230502_135032_MINI3PRO_20m"
	 ...
	 #   foreach(y = 1:length(table2[,1])) %dopar% {
			selectRow=table2[y,]
		  ...
		  fig <- image_graph(width = 1084, height = 1084, res =720) #1084 +20
		  par(mai=c(0,0,0,0),bg=NA,fig=c(0,1,0,1),bty ="n") 
		  

		  plot(pts,xlim=xlim,ylim=ylim, col="red",
			   pch=16,bg=16,axes=F, ann=F, xaxt='n', yaxt='n')	 #frame.plot=F,
		  dev.off()
		   fig=image_crop(fig,"1024x1024+30+30+30+30") #30
		   fig = image_resize(fig, "1024x1240!")
		  fig=image_crop(fig,"1024x1024+0+108+0+108") #30
		...
		# image=image_read(img_from)
		# Check=image_composite(image,fig,operator = "blend", compose_args="50")
		# Check
			}
		  }
