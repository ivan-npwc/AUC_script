library(rgdal)
 ...
 USE_MODEL_GAREM_MALE_COUNT = FALSE
...
#source("Modules/KMLwrite_function.r")
 ...
 #  difference=read.csv(save_pth_check_diff)
  ...
  if (USE_MODEL_GAREM_MALE_COUNT == TRUE){ 
      APO3= APO2[!is.na(over(APO2,as(ModelPoligon1,"SpatialPolygons"))),]                    # if use model NFS Garem Male Count 
      OBS = Observer[!is.na(over(Observer,as(ModelPoligon1,"SpatialPolygons"))),]
  } 
  if (USE_MODEL_GAREM_MALE_COUNT == FALSE){ 
      APO3=APO2
      OBS = Observer
   }
