...
ObserverPointPTH=list.files(ObserverPointDIR,full.names=T,pattern="shp|kml")[1] 
 ...
 #error_calculate=function(FP_FN=TRUE){
...
##################################################################################################################				
##############################################################################################################		
 if (Species %in% c("NFSadult","NFSAdult" ,"SSLAdult","NFSPup256","NFSPup512")) {		
	...
	if (length(RookeryPolygonPTH) !=0)	 {RookeryPol=readOGR(RookeryPolygonPTH);proj4string(RookeryPol) <- crs}
		 ...
		 if(Species=="NFSPup256"){PredictPoint1=PredictPoint1[PredictPoint1$Description=="P",]} 
		  
		  ObserverCount0 = ObserverPoint[!is.na(over(ObserverPoint,as(ModelPoligon1,"SpatialPolygons"))),]
		  ##############################################################################################
        #  ObserverCount0$LAYER	= 	ObserverCount0$Description 
	...
	#	 stsf=sf::st_as_sf(PredictCount)
		# write_sf(stsf,PredModelCount_pth,driver="kml")
	 ...
	 ObserverCount$age= gsub("SA","Bch",ObserverCount$age)
	  ObserverCount$age= gsub("J","Bch",ObserverCount$age)
	    ...
	    Compare1$diffPerc=Compare1$diffInd/Compare1$ObserverCount*100
			 ...
			 TottalErrorIndivid = autocountAdult -observerCountAdult
			 TottalErrorPercent=TottalErrorIndivid/observerCountAdult*100
	
	

Report=list(date1=basename(labelInput),
           observerCountAdult=observerCountAdult,
           autocountAdult=autocountAdult,
	...
	a =as_tibble(Report$Compare1) # 
	a$AutoCount[is.na(a$AutoCount)==T]=0
	a$age[a$age %in% c("AN","Bch")]= "Haulout"
	a$age[a$age %in% c("TF","F")]= "Garem"
	library(dplyr)
	b= a  %>%
	   #select(age, AutoCount,ObserverCount) %>%
	   group_by(age) %>%
	   summarise(AutoCount=sum(AutoCount),ObserverCount=sum(ObserverCount))
	   b$diffInd= b$AutoCount - b$ObserverCount
       b$diffPerc=	 b$diffInd/b$ObserverCount *100
	print(b)
	print(as_tibble(Report$Compare1))
	print(as_tibble(data.frame(observerCountAdult=observerCountAdult,TottalErrorIndivid=TottalErrorIndivid,TottalErrorPercent=TottalErrorPercent)))
	if (abs(b$diffPerc[b$age=="Garem"])>10){stop ("Please setting up Blob Analysis line 92-94")}
	#if (FP_FN==TRUE){source("C:\\Users\\usato\\SSL_DB\\AUC\\Modules\\FP_FN.r")}
