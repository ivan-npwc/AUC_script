...
EffortDay=  Effort[Effort$date== basename(labelInput),]
 ...
 LatLon=Info[Info$Property %in% c("Exif/GPSLongitude","Exif/GPSLatitude"),]
 ...
 InfoDayTable=NULL
   ...
   preTb=data.frame(file_name=imgN,latitude=lat,longitude=lon)
   InfoDayTable=rbind(preTb,InfoDayTable)
   }
  InfoDayTable$altitude=a
} 
#####################################################################################
...
if (EffortSort$type_count==	"Manual_model_count") { 
...
polygons_model_sites$species=species 
polygons_model_sites$r_date=r_date
polygons_model_sites$time_start=time_start
polygons_model_sites$projection=projection           
  
			qlite    <- dbDriver("SQLite")
