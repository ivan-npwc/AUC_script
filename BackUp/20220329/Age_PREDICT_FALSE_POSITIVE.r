...
##################################################################################
	...
	if (Species=="SSLPup"){
	     mdlBSAgePTH =paste0(System_data,"/weights/FPage/0220329_loss_0.19_epoch_04.h5")
         WeightPTH=   paste0(System_data,"/weights/FPage/PupSSLFP-TP")
         Dir=paste0(labelInput,"\\Predict\\Age_predict")
		 hdr=paste0(Dir,"\\Haulout")
		 rdr=paste0(Dir,"\\Rookery")
		 ListImgs1=list.files(rdr,full.names=T,recursive=TRUE)
		 ListImgs2=list.files(hdr,full.names=T,recursive=TRUE)
		 ListImgs=c(hdr,rdr)
	}
