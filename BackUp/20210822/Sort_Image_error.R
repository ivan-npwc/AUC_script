...
listImgError=list.files(DirImgError)
 ...
 SpP_data=create_tile_polygons(labelInput)
 
pointsError=shapefile(pointsPth)
proj4string(pointsError) <- crs
pointsError=spTransform(pointsError,CRS(crs))
 
 index= SpP_data %over% pointsError
...
for (i in 1:length(imgList4)) {imgList4[i]=gsub(paste0(basename(labelInput),"_"),"",imgList4[i])}
...
SpP=SpatialPolygons(srPolygons)
	...
	proj4string(SpP_data1) <- CRS("+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0")
	SpP_data1=spTransform(SpP_data1,CRS(crs))
	SpP_data1
	
	oregon.coords <- coordinates(SpP_data1)
	oregon.id <- cut(oregon.coords[,1], quantile(oregon.coords[,1]), include.lowest=TRUE)
	
	
	SpP_data2=unionSpatialPolygons(SpP_data1,oregon.id )
	SpP_data3= SpatialPolygonsDataFrame(SpP,data1[1:length(SpP_data2),])

 writeOGR(SpP_data2,  SavePTHpol, layer ="Error1", driver="KML")       #ESRI Shapefile
